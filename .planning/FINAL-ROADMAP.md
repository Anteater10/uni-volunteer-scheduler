...
- ~~**No `/health` endpoint**~~ ❌ **THIS CLAIM WAS WRONG.** `main.py:103` serves `GET /api/v1/health` and pings the DB; `docker-compose.prod.yml` already healthchecks it every 15s. The original grep looked for `"/health"` and missed the `/api/v1` prefix.
