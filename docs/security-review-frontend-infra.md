...
> **Status note added 2026-09-08, when this review was landed on `main`.** The review is
> a snapshot of 2026-08-20 and parts of it have since been actioned; read the findings as
> history, not as current state.
>
> - **F1** — the 14-day refresh window it flags is now **2 days**, set in the same commit
>   that added this file. That is a compensating control only. Gate 0 #2 (2026-09-07)
>   ruled that token storage itself gets fixed properly — `HttpOnly` cookie plus
>   in-memory access token, exactly as recommended below — in Phase L3. PR #79, which
>   proposed *accepting* `localStorage` instead, was closed for that reason.
> - **F2** — the CSP and the framing/type/referrer headers it reports as missing are now
>   live in `frontend/nginx.conf` and `backend/app/main.py`. Gate 0 #3 confirmed this;
>   `Caddyfile:36` is still HSTS-only, but it is no longer the only place headers are set.
