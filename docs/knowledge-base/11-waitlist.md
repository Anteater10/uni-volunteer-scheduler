...
## Only a deliberate staff promotion sends anyone the confirm-your-spot email

**Cancelling, capacity raises, and the hourly cleanup all free a seat, but none of them promote
anyone off the waitlist.** The seat just sits open until a staff member acts. The promotion email
goes out only when:
...
Both land the volunteer at **pending** with the same three-day confirm email — moving or swapping a
waitlisted volunteer onto an open session is a staff decision on their behalf, not something the
volunteer asked for at that moment, so it goes through the same offer-and-confirm step as any other
promotion. A staff **move** also frees the seat the volunteer moved *from* — permanently; that seat
doesn't backfill either.
...
**Confirm my spot**. It tells them that if they can't make it, they can just ignore the email — the
offer expires on its own — and that for any other change they should contact the SciTrek organizers;
the link keeps working for viewing their signups after they confirm.

## If they don't confirm, the offer lapses

**Three days after the email, an unclaimed offer is withdrawn and the seat is freed again.**
...
- The seat is freed and **stays open**. Nobody is promoted into it automatically — a staff member has
  to promote the next person on the waitlist by hand if the seat should be filled.
...
answer is almost always that more than three days passed and the offer was withdrawn — the seat is
...
**checking a volunteer in confirms them automatically** — a pending volunteer who shows up and gets
...
A waitlisted signup can also just be **cancelled** by staff — a volunteer who wants off a waitlist
emails the organizers and staff cancel it from the roster. The email that goes out then says they've
been **removed from the waitlist**, not that a signup was cancelled — they never held a seat, so the
seat-holder wording would be misleading.
